...
import { ToolSourceAuthError, type ToolSourceContext, ToolSourceUnavailableError } from "@nestjs-adk/core";
...
const CONTEXT: ToolSourceContext = { agentName: "agent", signal: new AbortController().signal };
...
describe("AdkMcpServer: how a failed connection is classified", () => {
	...
	it("turns a refused credential into a request to re-authorize", async () => {
		...
		await expect(server.open(CONTEXT)).rejects.toBeInstanceOf(ToolSourceAuthError);
	});
	...
	it("keeps a server that is merely down out of the re-authorize bucket", async () => {
		...
		await expect(server.open(CONTEXT)).rejects.toBeInstanceOf(ToolSourceUnavailableError);
	});
	...
	it("still closes after a failed handshake", async () => {
		...
		await expect(server.open(CONTEXT)).rejects.toThrow();
		await server.close();
