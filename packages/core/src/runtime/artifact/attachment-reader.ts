...
/**
 * Brings attachments back from storage so a message can be read as it was sent.
 *
...
export class AttachmentReader {
	...
	public async read(sessionId: SessionId, ids: readonly ArtifactId[]): Promise<readonly MediaPart[]> {
		const parts: MediaPart[] = [];
		for (const id of ids) {
			const part = await this.one(sessionId, id);
			if (part !== undefined) parts.push(part);
		}
		return parts;
	}

	private async one(sessionId: SessionId, id: ArtifactId): Promise<MediaPart | undefined> {
