...
describe("Ticket", () => {
	it("keeps what was complained about and when", () => {
		const ticket = Ticket.of("T-1", "A-1042", "the controller arrived broken", "2026-08-05T00:00:00.000Z");

		expect(ticket.id).toBe("T-1");
		expect(ticket.orderId).toBe("A-1042");
		expect(ticket.reason).toBe("the controller arrived broken");
	...
	it("has no conversation when it was opened on the site", () => {
		expect(Ticket.of("T-1", "A-1042", "broken", "2026-08-05T00:00:00.000Z").fromConversation).toBe(false);
	});
});
