 ...
 * green tick says nothing about that. This turns the event stream into a transcript:
 * the question, every tool the model reached for with the arguments it chose, what came
 * back, and the sentence it finally wrote.
 *
 * It is a consumer like any other, so it costs no call and sees exactly what an
 * application observing its own runs would see.
 */
export class Transcript extends SessionEventConsumer {
